...
var currentDemo,
  ...
  samples = {
   'flipbook1' : {'path' : 'samples/flipbook1/'},
   'flipbook2' : {'path' : 'samples/flipbook2/'},
   'flipbook3' : {'path' : 'samples/flipbook3/'},
   'flipbook4' : {'path' : 'samples/flipbook4/'},
   'flipbook5' : {'path' : 'samples/flipbook5/'},
   'flipbook6' : {'path' : 'samples/flipbook6/'},
   'flipbook7' : {'path' : 'samples/flipbook7/'},
   'flipbook8' : {'path' : 'samples/flipbook8/'},
   'flipbook9' : {'path' : 'samples/flipbook9/'},
   'flipbook10' : {'path' : 'samples/flipbook10/'}
  },
  status = {
    'unloaded' : 0,
    'loading' : 1,
  ...
  Hash.on('samples\/([^\/]+)(?:\/([0-9]*))?$', {
    ...
    nop: function(path) {
      if (!(new RegExp(sections)).test(path)) {
        if (currentDemo)
          samples[currentDemo].startPage = 1;
        bookshelf.close();
        demoHash = '';
      }
    }
