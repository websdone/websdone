...
var sampleName = 'flipbook10',
	samplePath = 'samples/flipbook10/';
...
function loadPage(page) {
	...
	var img = $('<img />');
	img.load(function() {
		var container = $('.flipbook10 .p'+page);
		img.css({width: '100%', height: '100%'});
		img.appendTo($('.flipbook10 .p'+page));
...
function loadFlipbook(flipbook) {
	...
	flipbook.turn({
		...
		turning: function(e, page, view) {
			...
			$('.next-button').removeClass('btn-right');
			...
			if (page==1)
				$('.next-button').addClass('btn-right');
			else
				$('.previous-button').removeClass('btn-right');
			...
			if (page==pages)
				$('.previous-button').addClass('btn-prev');
			else
				$('.previous-button').removeClass('btn-prev');
		...
		when: {
			...
			change: function(event, scale) {
				...
				if (scale==1) {
					...
					$('.flipbook10').removeClass('animated').addClass('zoom-in');
			...
			zoomOut: function () {
				...
				setTimeout(function(){
					$('.flipbook10').addClass('animated').removeClass('zoom-in');
				}, 0);
			...
			swipeLeft: function() {
				...
				$('.flipbook10').turn('next');

			},

			swipeRight: function() {
				
				$('.flipbook10').turn('previous');
...
bookshelf.loadSample(sampleName, function(action) {
	...
	if (!sample.flipbook) {
		...
		var bookClass = (Modernizr.csstransforms) ?
			'mag1-transform flipbook10' :
			'flipbook10';

		sample.flipbook = $('<div />', {'class': bookClass}).
			html('<div ignore="1" class="next-button next-button-hover"></div> <div ignore="1" class="previous-button previous-button-hover"></div>').
			appendTo($('#book-zoom'));
